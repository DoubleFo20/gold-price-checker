# Task Assignment: Worker M1 (Agent A - Forecasting & Statistics Engineer)
...
1. **Frontend Dropdown (`components/6-forecast.html`)**:
   - Add `<option value="30">30 วัน (1 เดือน)</option>` and `<option value="90">90 วัน (3 เดือน)</option>` to `#forecast-period` dropdown.
2. **Forecast Routes (`api/routes/forecast_routes.py`)**:
   - Expand `SUPPORTED_PERIODS` from `(1, 7, 30)` to `(1, 7, 30, 90)`. Import `SUPPORTED_PERIODS` from `services.forecast_service`.
   - Maintain route `/api/forecast` returning 200 OK with full payload.
3. **Forecast Service (`api/services/forecast_service.py`)**:
   - Expand `SUPPORTED_PERIODS = (1, 7, 30, 90)`.
   ...
   - Update evaluation payload (`_evaluation_payload`): handle 90-day interval with fallback to ensure valid accuracy metrics.
...
4. **Test Adjustments (`tests/e2e/test_tier2_boundaries.py`)**:
   - Update `test_b09_insufficient_historical_data_returns_503` to assert that when data is insufficient, it now returns 200 OK with valid bootstrap forecast fallback instead of HTTP 503.

## Mandatory Verification Command
You MUST execute the test suite to verify your implementation:
```powershell
.venv\Scripts\python.exe -m pytest tests/
```
Verify that all tests pass without errors. Document test output in your handoff.

## MANDATORY INTEGRITY WARNING
DO NOT CHEAT. All implementations must be genuine. DO NOT hardcode test results, create dummy/facade implementations, or circumvent the intended task. A teamwork_preview_auditor will independently verify your work. Integrity violations WILL be detected and your work WILL be rejected.

## Deliverable
Write your implementation summary to `d:\xampp\htdocs\gold-price-checker\.agents\worker_m1_opt\changes.md` and complete `handoff.md` with passing test results. Send a message back when complete.

## 2026-09-09T22:00:09Z
Read your task assignment in d:\xampp\htdocs\gold-price-checker\.agents\worker_m1_opt\DISPATCH.md.
